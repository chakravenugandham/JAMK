...
export class AppService {
  ...
  postService(token, data) {
    ...
    return this._http.post(this.host + this.url, data, { headers: headers }).pipe(map(res => res, err => err.json()));
}
...
putService(token, data) {
    ...
    headers.append('Content-Type', 'application/json; charset=utf-8');
    headers.append('Authorization', 'Bearer ' + token);
    return this._http.put(this.host + this.url, data, { headers: headers }).pipe(map(res => res));
}
...
deleteService(token, data) {
 ...
 //   data = { "id": "9c3f083f-34c5-444d-a9a7-26c4b3f36fc6", "leaveTypeCode": "CO", "from": 1512649604444, "to": 1512649600000, "employeeId": 1219 }
    return this._http.delete(this.host + this.url, { headers: headers }).pipe(map(res => res));
}
